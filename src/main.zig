...
const Task = struct {
    []const u8,
    union(enum) { app: []const u8, menu: []const u8 },
};
...
fn eventTapCallback(proxy: c.CGEventTapProxy, type_: c.CGEventType, event: c.CGEventRef, userInfo: ?*anyopaque) callconv(.c) c.CGEventRef {
    ...
    if (type_ == c.kCGEventKeyDown) {
        ...
        if (show_key_text) {
            if (std.mem.startsWith(u8, key_str_zig, "⌘") or
                ...
                std.mem.startsWith(u8, key_str_zig, "⌃"))
            {
                ...
                c.show_text_for_duration(new_str, 1.5);
            } else {
                const allocator = std.heap.page_allocator;
                const new_str: [:0]u8 = if (key_str_zig.len > 1)
                    std.fmt.allocPrintSentinel(allocator, " {s} ", .{key_str_zig}, 0) catch unreachable
                else
                    std.fmt.allocPrintSentinel(allocator, "{s}", .{key_str_zig}, 0) catch unreachable;
...
fn createCFString(c_str: [*:0]const u8) c.CFStringRef {
    const cf_str = c.CFStringCreateWithCString(c.kCFAllocatorDefault, c_str, c.kCFStringEncodingUTF8);
...
const Key = struct {
    ...
    const Mask = struct { command: bool = false, shift: bool = false, control: bool = false, alternate: bool = false };
    fn sleep10ms() void {
...
const Mouse = struct {
    ...
    pub fn post_event(self: Self, mouse_type: c_uint, button: u32) !void {
        const mouse_event = c.CGEventCreateMouseEvent(self.source, mouse_type, self.point, button);
...
const AppInfo = extern struct {
    ...
    pub fn format(self: @This(), comptime fmt: []const u8, options: std.fmt.FormatOptions, writer: anytype) !void {
        _ = fmt;
        _ = options;
        try writer.print("AppInfo{{.ref={?},.pid={},.name=\"{s}\"}}", .{ self.ref, self.pid, self.name });
    }
};
const ValueItem = extern struct {
    ...
    pub fn format(self: @This(), comptime fmt: []const u8, options: std.fmt.FormatOptions, writer: anytype) !void {
        _ = fmt;
        _ = options;
        try writer.print("ValueItem{{.ref={?},.name=\"{s}\"}}", .{ self.ref, self.name });
    }
};
